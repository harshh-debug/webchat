...
const ChatSidebar = ({
  ...
  return (
    <aside className={cn(
      ...
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-zinc-200 dark:border-white/[0.06]">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-white dark:bg-white flex items-center justify-center shadow-sm border border-zinc-200 dark:border-transparent">
          ...
          <span className="font-semibold text-[14px] text-zinc-900 dark:text-white tracking-tight">
            {showAllUsers ? "New Chat" : "WebChat"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            size="icon"
            className={cn(
              "h-8 w-8 rounded-lg text-sm transition-colors duration-150",
              showAllUsers
                ? "text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10"
                : "text-zinc-400 hover:text-zinc-700 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/[0.06]",
            )}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="sm:hidden h-8 w-8 rounded-lg text-zinc-400 hover:text-zinc-700 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/[0.06] transition-colors"
        ...
        <div className="relative">
          ...
          <Input
            ...
            className={cn(
              ...
              "focus-visible:ring-1 focus-visible:ring-white/20 dark:focus-visible:ring-white/10",
      ...
      <ScrollArea className="flex-1 px-2">
        {showAllUsers ? (
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    "transition-all duration-150",
                    isSelected
                      ? "bg-zinc-100 dark:bg-white/[0.07] ring-1 ring-zinc-200 dark:ring-white/[0.08]"
                      : "hover:bg-zinc-100/70 dark:hover:bg-white/[0.04]",
                  )}
                >
                  <div className="relative shrink-0">
                    <Avatar className="w-10 h-10">
                      <AvatarFallback className={cn(getAvatarColor(chat.user._id), "text-white text-[13px] font-semibold")}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between gap-1 mt-0.5">
                      <p className={cn(
                        ...
                        unseenCount > 0 ? "text-zinc-700 dark:text-zinc-300 font-medium" : "text-zinc-400 dark:text-zinc-500"
                      )}>
                        {latestMessage
                      ...
                      {unseenCount > 0 && (
                        <Badge className="h-[18px] min-w-[18px] px-1.5 text-[10px] font-bold bg-white dark:bg-white text-[#0a0a0a] hover:bg-white rounded-full shrink-0">
          ...
          <div className="flex flex-col items-center justify-center h-52 text-center px-4">
            <div className="w-12 h-12 rounded-2xl bg-zinc-100 dark:bg-white/[0.05] flex items-center justify-center mb-3">
            ...
            <p className="text-[13px] font-medium text-zinc-600 dark:text-zinc-300">No conversations yet</p>
            <p className="text-[12px] text-zinc-400 mt-1 leading-relaxed">
              Hit <span className="font-semibold text-zinc-600 dark:text-white">+</span> to start a new chat
            </p>
          </div>
        )}
      </ScrollArea>
          ...
          <div className="flex items-center gap-3 px-3 py-2 rounded-xl mb-1">
            <div className="relative shrink-0">
              <Avatar className="w-8 h-8">
                <AvatarFallback className={cn(getAvatarColor(loggedInUser._id), "text-white text-[11px] font-semibold")}>
                  {getInitials(loggedInUser.name)}
                </AvatarFallback>
              </Avatar>
              <span className="absolute bottom-0 right-0 w-2 h-2 rounded-full bg-emerald-500 ring-2 ring-white dark:ring-[#111214]" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-semibold text-zinc-800 dark:text-zinc-100 truncate">{loggedInUser.name}</p>
